...
class Newton {
    ...
    public Newton(Polynomial p) {
        ...
        this.f = p; //Does it matter that I'm setting f to have the same reference as the polynomial that I'm passing in? Assume not.
    ...
    public void iterate(Complex z0) {
        // You need to fill in this method.
        Complex zn = z0; //sets the initial value of zn to z0
        Complex zn1;
        for(numIterations = 0; numIterations < MAXITER; numIterations++){
        	if(fp.evaluate(zn).abs() < TOL){ //if derivative goes to zero, sets error to -1 and returns
        		err = -1;
        		return;
        		}
        	zn1 = f.evaluate(zn).divide(fp.evaluate(zn)).minus().add(zn); //sets z_(n+1) as appropriate
        	if(zn1.add(zn.minus()).abs() < TOL){ 
        		root = zn1; // if the two roots are less than TOL apart, we say that z_(n+1) is the root
        		err = 0; //no error :)
        		return;
        		}
        	zn = zn1; //set zn to zn1 so that we can iterate again
        	}
        err = -2;
        //if we don't return a root at some point in the loop, then we set err as -2 and return
        return;
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(n.getNumIterations());
