...
class NewtonFractal {
    ...
    public void printRoots() {
        ...
        for(int i = 0; i < roots.size(); i++){
        	System.out.println("The " +i+ "th root in the array list is " + roots.get(i));
        	}
    }
    ...
    public int findRoot(Complex root) {
        // You need to fill in this function.
        for(int i = 0; i < roots.size(); i++){
        	if(root.add(roots.get(i).minus()).abs() < Newton.TOL){ // checks if root in array to within tolerance
        		return i; //if so, returns position of root
        		}
        	}
     ...
     *
     * @param i  x-axis co-ordinate of the pixel located at (i,j) (column)
     * @param j  y-axis co-ordinate of the pixel located at (i,j) (row)
     */
    ...
    public void createFractal(boolean colorIterations) {
      ...
      for(int j = 0; j < NUMPIXELS; j++){ //go through each row
      	for(int k = 0; k < NUMPIXELS; k++){ //go through each column
      		Complex pix = pixelToComplex(k,j); //translate pixel to a complex number
      		iterator.iterate(pix); //use newton-raphson on this complex numbers
      		if(iterator.getError() == 0){ //if no error after running newton-raphson
      			Complex iteratedRoot =  iterator.getRoot();
      			int col = findRoot(iteratedRoot); //check if root has been found
      			if(col == -1){
      				this.roots.add(iteratedRoot); //if not, add root to arraylist
      				col = findRoot(iteratedRoot); //recheck if root is in arraylist, so we know what colour to give it
      				}
      			colorPixel(k,j,col,iterator.getNumIterations()); //colour the pixel
      			}
    ...
    public static void main(String[] args) {
        ...
        Complex[] coeff2 = new Complex[] {new Complex(1), new Complex(), new Complex(1), new Complex(), new Complex(1)};
        Polynomial q = new Polynomial(coeff2);
        NewtonFractal g = new NewtonFractal(q, new Complex(-1,1),2);
        g.createFractal(true);
        g.saveFractal("myfractaldark.png");
        g.createFractal(false);
        g.saveFractal("myfractallight.png");
    }
